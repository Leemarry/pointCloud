  ...
  <div ref="container" class="container" :style="{height:conHeight}" @scroll="handleScroll">
    <div class="list" :style="{top:listTop}">
      <div v-for="(item) in showData" :key="item.id" :style="{height:size+'px',display:'flex'}" class="items">
        ...
        <el-col :span="12">
          <div style="display: flex;align-items: center;"> <i style="color:#409EFF" class=" el-icon-s-order" />{{ item.file.webkitRelativePath == "" ? item.path : item.file.webkitRelativePath }}</div>
        </el-col>
        <el-col :span="6">
            <template v-if="item.status === 4 || item.progress == 100">上传成功！</template>
