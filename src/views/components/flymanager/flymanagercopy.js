 ...
 * @version: 
 * @Author: Eugene
 * @Date: 2023-11-22 14:06:39
 * @LastEditors: Andy
 * @LastEditTime: 2023-11-23 17:33:59
 */
/*
 * @Descripttion: 
 * @version: 
 * @Author: Eugene
 * @Date: 2023-11-21 09:05:33
 * @LastEditors: Andy
 * @LastEditTime: 2023-11-21 09:05:34
 */
import {
  mapGetters
} from 'vuex'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      wpsGotoWaypointModeOptions: [{
        label: '空',
        value: 0
      }],
      wpsHeadingModeOptons: [{
        label: '空',
        value: 0
      }],
      wpsFlightPathModeOptions: [{
        label: '空',
        value: 0
      }],
      wpsFinishedActionOptions: [{
        label: '空',
        value: 0
      }],
      wpsRcSignalLostOptions: [{
        label: '空',
        value: 0
      }],
      djiMissionVersion: [{
  ...
  methods: {
      ...
      if (this.beforeKmlUpload(file.raw)) {
        ...
        reader.onload = function () {
          ...
          for (let index = 0; index < points.length; index++) {
            ...
            if (firstPoint === null) {
              firstPoint = {
                lat: positionAmap.lat,
                lng: positionAmap.lng
              }
            }
    ...
    getCommandDes(wp) {
      ...
      var item = this.efCommands.find(a => {
        return a.cmdId === cmd
      })
      if (item) {
    ...
    mapClick(mapsEvent) {
      const positionAmap = {
        lat: mapsEvent.lnglat.lat,
        lng: mapsEvent.lnglat.lng
      }
    ...
    enableEditWps(enable) {
      ...
      this.$notify({
        title: '',
        message: this.isEditWps ? '您可以开始编辑航线任务了。' : '已结束航线编辑模式！',
        type: this.isEditWps ? 'success' : 'info'
      })
    },
    clearWps() {
      this.wpsPath = []
    ...
    clickUavItem(item, needclose) {
      ...
      console.log(this.selectedUavItem);
    ...
    async initMyUavHives() {
      ...
      this.uavs = []
      // 调用的是src/store/modules中的login方法
      return this.$store.dispatch('user/getMyUavHiveVideo').then((response) => {
        const {
          code,
          data,
          message
    ...
    async addTaskWps(name, des) {
      ...
      this.loading = true
      // 调用的是src/store/modules中的login方法
      return this.$store.dispatch('efTaskWps/addTaskWps', {
    ...
    async deleteTaskById(id) {
      ...
      await this.$store
        .dispatch("efTaskWps/deleteTimedTask", {
          id: id
        })
        .then((response) => {
    ...
    async queryAllTaskWps() {
      ...
      this.taskWps = [];
      await this.$store
        .dispatch("timedTask/queryAllTaskWps", {
          type: null
        })
        .then((response) => {
          this.loadingWps = false;
    ...
    async queryByWpsIndex(id, wpsName, item) {
      ...
      await this.$store
        .dispatch("efTaskWps/queryByWpsIndex", {
          id: id,
        })
        .then((response) => {
          this.loadingWps = false;
    ...
    async getLineStreamOnline(url, type) {
      ...
      try {
        await this.$store
          .dispatch("efTencent/getLineStreamOnline", {
            appName: 'live',
            streamName: url,
            timeout: 5
          })
    ...
    websocketonmessage(e) {
      ...
      switch (messageID) {
            ...
            if (isNowUav) {
              ...
              if (data.lat === 0 && data.lng === 0) {
                ...
                if (this.selectedUavItem.heartbeatData.aremd === 0) {
                  this.selectedUavItem.homePosition = {
                    lat: data.lat,
                    lng: data.lng
                  }
                }
        ...
        case 2250: // 大疆服务后台心跳包
          ...
          const {
            versionInside, bootTime, isAirLinkOnline, isCameraOnline, isGcsOnline, isRomoteControlOnline, isUavOnline, machineCode
          } = data
