...
<template>
    <div class="layer-manager-box" id="layer-manager-box">
        ...
        <div id="layerTree" class="graphic-draw-layer-manager-class">
            <el-tree :data="json" show-checkbox node-key="id" ref="tree" @check="checkAction" :default-expanded-keys="['marker', 'polyline', 'polygon']">
                <template #default="{ data,node  }">
                    <div class="custom-tree-node">
                        <i v-show="data.icon" :class="data.icon" class="action-item"></i>
                        <!-- <span class="cecece"  @click="cese(data)">o1</span> -->
                        <!-- <span class="cecece" @click="cese(node,data)">o1</span> -->
                        <el-input v-model="newName" class="inputText" v-if="data.edit" @keyup.enter.native="renameAction(data)"></el-input>
                        ...
                        <template v-if="node.level === 3">
                            ...
                            <div class="slidershow">
                                <!-- 通过 @change  没有用 @input-->
                                <el-slider @change="sliderValueSave(node,data)" @input="sliderValueInput(node,data)" @mousedown.native="handleMouseDown" v-if="data.slidershow" v-model="data.sliderValue" :min="5" :max="150"></el-slider>
                            </div>
                        </template>
                    </div>
                </template>
