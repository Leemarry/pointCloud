<template>
  <el-container>
    ...
    <el-main>
      <el-table v-loading="loading" element-loading-text="拼命加载中" :data="loginedInfo" style="width: 100%" size="mini" height="100%" empty-text="暂无登录记录！">
        <el-table-column type="expand">
          <template slot-scope="props">
            <el-form label-position="left" :inline="true" class="demo-table-expand">
              <el-form-item label="登录名">
                <span>{{ props.row.uloginName }}</span>
              </el-form-item>
              <el-form-item label="登录IP(内网)">
                <span>{{ props.row.uipLocal }}</span>
              </el-form-item>
              <el-form-item label="客户码">
                <span>{{ props.row.umachineCode }}</span>
              </el-form-item>
            </el-form>
          </template>
        </el-table-column>
        ...
        <el-table-column label="登录时间" width="150">
          <template slot-scope="props">
            {{ $dateUtil.convertMillSecToDateStr(props.row.uloginTime) }}
          </template>
        </el-table-column>
        <el-table-column label="在线时长" width="120">
          <template slot-scope="props">
            {{ $dateUtil.convertSecondToFrendly2(props.row.uonlineTime) }}
          </template>
        </el-table-column>
        <el-table-column label="退出登录时间" width="150">
          <template slot-scope="props">
            {{ $dateUtil.convertMillSecToDateStr(props.row.uloginOutTime) }}
          </template>
        </el-table-column>
        <el-table-column prop="uagent" label="代理">
        </el-table-column>
        <el-table-column prop="uipWww" label="登录IP(外网)" width="120">
        </el-table-column>
        <el-table-column label="状态" width="100">
          <template slot-scope="props">
            <el-tag v-if="props.row.ustatus===0" type="success" size="mini">正常登录</el-tag>
            <el-tag v-else type="danger" size="mini">登录失败</el-tag>
          </template>
        </el-table-column>
      </el-table>
    </el-main>
  ...
  methods: {
          ...
          try {
            return this.$store
              ...
              .then((response) => {
                ...
                if (code === 1) {
                  const index = this.loginedInfo.findIndex(
                    (item) => {
                      return item.id === row.id;
                    }
                  );
                  if (index >= 0) {
                    this.loginedInfo.splice(index, 1);
                  }
        ...
        .then((response) => {
          ...
          if (code === 1) {
            if (this.isInsert) {
              ...
              if (index >= 0) {
                this.loginedInfo.splice(index, 1, data);
              }
            }
            this.showToast(message);
            this.isShowDialogInsert = false;
    ...
    async queryAll() {
      ...
      try {
        ...
        await this.$store
          .dispatch('user/queryAllUserLogin', parms)
          .then((response) => {
            this.loading = false
            const { code, message, data } = response
            if (code === 1) {
              this.loginedInfo = data
            } else {
              this.showMessage(message, "warning")
            }
          })
