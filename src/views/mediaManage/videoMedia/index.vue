      ...
      <el-table :data="tableData.slice((currentPage - 1) * pageSize, currentPage * pageSize)" stripe style="width: 100%">
        ...
        <el-table-column fixed="right" label="操作" width="100">
          <template slot-scope="scope">
            <el-button @click="openVideoTag(scope.row)" type="text" size="small">查看</el-button>
...
export default {
  ...
  data() {
    ...
    for (let i = 1; i <= 200; i++) {
      ...
      const fileUrl =  `http://127.0.0.1:9090/ceshi1/video/layout-header.mp4`;
  ...
  methods: {
    ...
    openVideoTag(row) {
      // 窗口名
      const windowName = 'videoTag-' + row.fileName;
      if (!this.windowns[windowName] || this.windowns[windowName].closed) {
        ...
        this.windowns[windowName] = existingWindow;
      } else {
        this.windowns[windowName].focus();
      }
    },
    downloadVideo(row) {
      this.getMethod(row.fileUrl).then((blob) => {
