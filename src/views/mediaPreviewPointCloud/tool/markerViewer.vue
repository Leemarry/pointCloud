<!--
 * @Date: 2024-08-07 19:46:24
 * @LastEditors: likai 2806699104@qq.com
 * @FilePath: \pointCouldPages\src\views\mediaPreviewPointCloud\tool\markerViewer.vue
 * @Description: Do not edit
-->
<template>
  <div id="markerContainer" :style="{ left: winPos.x + 'px', top: winPos.y + 'px' }">
    <el-dialog v-model="dialogVisible" :title="markName + '附件'" :modal-append-to-body="false">
      <el-upload ref="upload" class="upload-class" :action="attachment.action" :on-remove="attachment.delete" :on-success="attachment.success" :file-list="attachment.fileList" :on-preview="attachment.download" :auto-upload="false">
        <template #trigger>
    ...
    <div v-if="markMode === 'marker'" v-show="visible" id="createMerkerPanel">
      <el-container v-show="!selectPanel">
        <el-header>
          <span>添加标记</span>
          <i v-if="attachment.enabled" class="el-icon-paperclip" title="附件" style="font-size:16px" @click="dialogVisible = true" />
        ...
        <el-container id="marker-panel">
          <el-container>
            <el-main class="marker-main-class">
              <div class="pane-row">
                <div>名称：</div>
                <el-input ref="nameinput" v-model="markName" @keyup.enter="update" />
              </div>
              <div class="pane-row pane-desc">
                <div>描述：</div>
                <el-input v-model="markRemark" type="textarea" />
              </div>
            </el-main>
            <el-aside>
              <img :src="selectedImage">
              <br>
              <a id="imageC" href="#" @click="selectPanel = true">更换</a>
            </el-aside>
          </el-container>
          <el-footer>
            <el-button id="cancelbtn" type="danger" plain size="small" @click="cancelMark">删除</el-button>
            <el-button id="submitbtn" type="primary" plain size="small" @click="update">确定</el-button>
          </el-footer>
        </el-container>
      </el-container>
      <el-container v-show="selectPanel" class="image-list-class">
        <!-- <img :src="noImage" @click="changeHandler(undefined)"> -->
        <img v-for="(img, index) in images" :key="index" :src="img" @click="changeHandler(img)">
      </el-container>
    </div>
...
export default {
    ...
    methods: {
        ...
        createLabelAndaddMarker(cartesian, options, markerid) {
            const self = this;
            const marker = self.createLabel(cartesian, options);
            self.addMarker(markerid, marker);
        },
        createLabel(cartesian, options) {
            if (this.markerManager) {
                return this.markerManager.createLabel(cartesian, options);
            }
        },
        addMarker(markerid, marker) {
            if (this.markerManager) {
                return this.markerManager.addMarker(markerid, marker);
            }
        },
        ...
        droplikeid(id) {
            if (this.markerManager) {
                this.markerManager.droplikeid(id);
            }
        },
        rename(id, name) {
            if (this.markerManager) {
        ...
        update() {
            console.log(this.markName, this.markRemark);
            if (this.markerManager) {
