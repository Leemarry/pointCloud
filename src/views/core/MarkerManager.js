 ...
 * @FilePath: \pointCouldPages\src\views\core\MarkerManager.js
...
export default class MarkerManager {
     ...
     */
        ...
        this.pickHandler = new Cesium.ScreenSpaceEventHandler(viewer.canvas); //创建一个 Cesium 的ScreenSpaceEventHandler 对象并将它绑定到 Cesium 的 Viewer 的画布（canvas）上
   ...
   */
    pick(type = 'marker', mode = 'single') {
        ...
        // this.cursorTip.distanceX = CursorTipDistance.distanceX;
        // this.cursorTip.distanceY = CursorTipDistance.distanceY;
        ...
        const pick = function(e) {
            const newX = e.position.x + 350;
            const newY = e.position.y + 100; //{ x: newX, y: newY } 移动输入框
   ...
   */
    ...
    /**
   * @name:
   * @msg: 创建了一个 Label 实体对象
   * @param {*} cartesian
   * @return {*}
   */
    createLabel(cartesian, options = CesiumLabel.defaultStyle) {
   ...
   */
    droplikeid(id) {
        const manager = this.manager
        manager.forEach((maker, makerid, map) => {
            if (makerid === id || makerid.includes(id)) {
                this.drop(makerid)
            }
        });
    // console.log(this.manager);
