...
class CesiumPolygon extends BaseGraphic {
  ...
  /**
   * 多边形面积
   *  @param {Array} positions  一个点的数组：每个对象都具有 x、y、z 三个属性，表示三维空间中的坐标点。
  ...
  calculatePolygonArea(positions) {
    ...
    for (var i = 0; i < length; i++) {
      ...
      var nextPoint = positions[(i + 1) % length]; // 获取下一个点（如果当前点是最后一个点，则下一个点为第一个点）
...
class CesiumLabel extends BaseGraphic {
  ...
  static defaultStyle = {
    color: Cesium.Color.fromCssColorString('rgba(0,255,0,0.8)'),
    fillColor: Cesium.Color.WHITE,
    font: '28px sans-serif',
    showBackground: true
  }
