    ...
    methods: {
        ...
        async playVideo1Tx(url) {
            if (url && url.length > 0) {
                ...
                if (StreamState !== null) {
                    ...
                    if (StreamState == 'flv' || StreamState == 'rtmp' || StreamState == 'm3u8') {
                        goon = true
                ...
                if (goon) {
                    ...
                    console.log('无人机视频播放地址：' + "webrtc://" + url)
                    console.log('无人机视频播放地址：' + "http://" + url + ".flv")
                    console.log('无人机视频播放地址：' + "rtmp://" + url)
                    this.TcPlayerUav = new this.TcPlayer('video1', {
                            // /** * 视频类型播放优先级 * mobile ：m3u8>mp4 * PC ：RTMP>flv>m3u8>mp4 */
                            "webrtc": "webrtc://" + url,
                            // "rtmp":"rtmp://" + url, // 播放 RTMP 格式的视频必须启用 Flash，目前浏览器默认禁用 Flash，需用户手动开启。
                    ...
                    if (this.selectedHiveItem.heartbeatData && this.selectedHiveItem.heartbeatData != null) {
                        switch (type) {
                            ...
                            case 'chargingText':
                                ...
                                //console.log('充电参数：' + ChargeVoltage + "," + ChargeCurrent + "," + ChargePert + "," + ChargeNeedTime)
                                // if (ChargeVoltage && ChargeCurrent && ChargePert && ChargeNeedTime) {
                                //     return '充电电压: ' + ChargeVoltage.toFixed(3) + '伏' +
                                //         ',\r\n 充电电流: ' + ChargeCurrent.toFixed(3) + '安' +
                                //         ',\r\n 电量百分比: ' + ChargePert.toFixed(0) + '%' +
                                //         ',\r\n 剩余时长: ' + ChargeNeedTime.toFixed(2) + '分钟'
                                // } else {
                                //     console.log("我进来了" + ChargeVoltage)
                                //     return '充电电压: 0伏' +
                                //         ',\r\n 充电电流: 0安' +
                                //         ',\r\n 电量百分比: 0%' +
                                //         ',\r\n 剩余时长: 0分钟'
        ...
        getHiveRealdataDes(type) {
            try {
                        ...
                        switch (type) {
                            case 'altabs':
                                return this.selectedHiveItem.heartbeatData.altabs.toFixed(2)
                                    // if (this.selectedHiveItem.heartbeatData.altabs) {
                                    //     return this.selectedHiveItem.heartbeatData.altabs.toFixed(2)
                                    // } else {
                                    //     return 0
                                    // }
                            case 'hivetemp':
                                return this.selectedHiveItem.heartbeatData.hiveTemperature.toFixed(2)
                                    // if (this.selectedHiveItem.heartbeatData.hiveTemperature) {
                                    //     return this.selectedHiveItem.heartbeatData.hiveTemperature.toFixed(2)
                                    // } else {
                                    //     return 0
                                    // }
                ...
                console.log('获取参数值 getHiveRealdataDes：' + type + " 异常：" + error)
            }
        },
        ...
        getIsUavServerOnline() {
            ...
            if (this.selectedUavItem) {
                // console.log('服务后台在线状态：2222222222 ,刷新时间为：' + this.selectedUavItem.djiServerTime + "，当前时间为：" + Date.now() + "，时间差为：" + (Date.now() - this.selectedUavItem.djiServerTime) + "毫秒");
                if (this.selectedUavItem.djiServerTime && this.selectedUavItem.djiServerTime + 10000 >= Date.now()) {
                    online = true;
                }
            }
            // console.log('服务后台在线状态：' + online);
            return online;
        },
        saveConfig(reset) {
        ...
        showNotify(msg, type) {
            // type 'success' : 'info'
            this.$notify({ title: '', message: msg, type: type })
        },
        switchVideoMap(type) {
            // if (this.bigView === 'map') {
            //     if (type === 'video1') {
            ...
            if (this.uavs && this.uavs.length > 0) {
                this.uavs.forEach((item, index) => {
                    ...
                    if (item.isDjiServerOnline && item.djiServerTime + 30000 < Date.now()) {
                        ...
                        console.log("遥控器在线状态：" + (item.isRomoteControlOnline));
                        isNeedForceUpdate = true
                    }
                })
            ...
            switch (messageID) {
                ...
                case 2250: // 大疆服务后台心跳包
                    ...
                    if (index >= 0) {
                        ...
                        console.log("遥控器在线状态：" + (isRomoteControlOnline === 1));
