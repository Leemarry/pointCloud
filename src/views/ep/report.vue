  ...
  <el-container>
    ...
    <el-header class="toolbar">
      ...
      <el-button-group size="mini" style="margin-left:10px" v-loading="loading">
        <el-button size="mini" type="" icon="el-icon-search" @click="queryData()">查询报告
        </el-button>
      </el-button-group>
    </el-header>
        ...
        <el-table ref="filterTable" v-loading="loading" :data="data" border style="width: 100%" :cell-style="tableCellStyle" @row-click="rowClick" @cell-mouse-enter="cellMouseEnter" @cell-mouse-leave="cellMouseLeave">
            ...
            <template slot-scope="props">
              <el-form label-position="left" inline class="demo-table-expand">
                ...
                <el-form-item label="时间">
                  <span>{{ parseTime(props.row.datadate) }}</span>
                </el-form-item>
                <el-form-item label="总飞行航程">
                  <span>{{ props.row.flightDistance }} 米</span>
                </el-form-item>
                <el-form-item label="总飞行时长">
                  <span>{{ props.row.flightDuration }} </span>
                </el-form-item>
              </el-form>
            </template>
          </el-table-column>
          ...
          <el-table-column label="日期" width="130">
            <template slot-scope="scope">
              <span>{{ scope.row.year }}年{{ scope.row.mouth }}月{{ scope.row.day}}日</span>
            </template>
          </el-table-column>
          <el-table-column label="起飞架次">
            <template slot-scope="scope">
              <span>{{ scope.row.flightSorties }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="areaFlyed" label="巡逻面积" />
          <el-table-column prop="areaFirePoint" label="火点总面积" />
          <el-table-column prop="numFirePoint" label="火点总数" />
          <el-table-column prop="areaBlackSpot" label="黑斑总面积" />
          <el-table-column prop="numBlackSpot" label="黑斑总数" />
          <el-table-column fixed="right" label="导出日报" width="130">
            <template slot-scope="scope">
              <el-button type="text" style="color:" @click="exportReport(scope.row,true)" size="small">内部日报</el-button>
              <el-button type="text" style="color:" @click="exportReport(scope.row,false)" size="small">对外</el-button>
            </template>
          </el-table-column>
          <el-table-column label="巡查单元">
            <template slot-scope="scope">
              <span>{{ scope.row.areaPoints }}</span>
            </template>
          </el-table-column>
        </el-table>
      </el-scrollbar>
    ...
    <el-dialog title="巡查区域" :visible.sync="dialogMapVisible" width="60%" @closed="closeImageOnMap">
    ...
    <el-dialog title="导出报告" :visible.sync="dialogExport" width="380px">
      <el-date-picker v-model="selectedDateDay" type="date" :picker-options="pickerOptionsDay" placeholder="选择日期">
      </el-date-picker>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      selectedDate: null,
      timeSelectRange: [new Date(Date.now() - 7 * 24 * 60 * 60 * 1000), new Date()],
      pickerOptions: {
    ...
    async queryData(becomeSilent) {
      try {
        ...
        await this.$store
          ...
          // .dispatch("efPhotoEp/queryAllByUavTime", model)
          .then((response) => {
            if (tips) {
              this.loading = false;
            }
